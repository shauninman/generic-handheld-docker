...
TOOLCHAIN_NAME=generic-handheld
...
shell: .build
	...
	docker run --privileged -it --rm -v "$(WORKSPACE_DIR)":/root/workspace $(TOOLCHAIN_NAME) /bin/bash
else
shell:
	$(info $(BOLD)Connecting to running $(TOOLCHAIN_NAME)...$(NORM))
	docker exec --privileged -it $(CONTAINER_NAME) /bin/bash  
endif
